...
inference_counter = meter.create_counter(
    name="ai.azure.inference.requests", description="AI inference requests in Azure", unit="1"
)

inference_duration = meter.create_histogram(
    name="ai.azure.inference.duration", description="AI inference duration", unit="ms"
)
    ...
    def process_request(self, user_input, user_id=None):
        ...
        with tracer.start_as_current_span("process_request") as span:
            ...
            try:
                ...
                inference_counter.add(1, {"model": self.model_name, "status": "success", "cloud": "azure"})

                inference_duration.record(duration_ms, {"model": self.model_name})

                # Add custom event
                span.add_event("Request processed successfully", {"duration_ms": duration_ms, "tokens": tokens_used})
                ...
                inference_counter.add(1, {"model": self.model_name, "status": "error", "cloud": "azure"})
...
def simulate_azure_workload():
    ...
    # Batch processing
    print("\nProcessing batch...")
    batch_inputs = ["Batch item 1", "Batch item 2", "Batch item 3"]
