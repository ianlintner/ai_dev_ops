...
inference_requests_total = Counter(
    "ai_inference_requests_total", "Total number of AI inference requests", ["model", "environment", "status"]
)
...
tokens_used_total = Counter("ai_tokens_used_total", "Total number of tokens used", ["model", "token_type"])

# Gauge for active inference requests
active_inferences = Gauge("ai_active_inferences", "Number of currently active inference requests", ["model"])
...
inference_errors_total = Counter(
    "ai_inference_errors_total", "Total number of inference errors", ["model", "error_type"]
)

# Gauge for model cost
inference_cost_dollars = Counter("ai_inference_cost_dollars_total", "Total cost of inference in dollars", ["model"])

# Info metric for model metadata
model_info = Info("ai_model", "Information about the AI model")
...
class AIModelMonitor:
    ...
    def __init__(self, model_name="gpt-4", environment="production"):
        ...
        # Set model info
        model_info.info(
            {"model_name": model_name, "version": "1.0.0", "environment": environment, "provider": "openai"}
        )
    ...
    def run_inference(self, input_text):
        ...
        try:
            ...
            with inference_latency_seconds.labels(model=self.model_name, environment=self.environment).time():
                # Simulate inference
                ...
                token_generation_rate.labels(model=self.model_name).observe(rate)

                # Record token usage
                tokens_used_total.labels(model=self.model_name, token_type="input").inc(input_tokens)

                tokens_used_total.labels(model=self.model_name, token_type="output").inc(output_tokens)

                # Calculate and record cost (example: $0.00002 per token)
                total_tokens = input_tokens + output_tokens
                cost = total_tokens * 0.00002
                inference_cost_dollars.labels(model=self.model_name).inc(cost)

                # Record successful request
                inference_requests_total.labels(
                    model=self.model_name, environment=self.environment, status="success"
                ).inc()
            ...
            # Record error
            inference_errors_total.labels(model=self.model_name, error_type=type(e).__name__).inc()

            inference_requests_total.labels(model=self.model_name, environment=self.environment, status="error").inc()
...
def simulate_ai_workload():
    ...
    models = [AIModelMonitor("gpt-4", "production"), AIModelMonitor("claude-2", "production")]
        ...
        try:
            ...
            if random.random() < 0.05:
                try:
                    ...
                    inference_errors_total.labels(model=model.model_name, error_type="ValueError").inc()
